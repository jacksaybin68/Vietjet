    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col xl:flex-row gap-4">
        <div className="relative flex-1 group">
          <Icon name="MagnifyingGlassIcon" size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
          <input
            type="text"
          ...
          {['all', 'completed', 'pending', 'failed', 'refunded'].map((s) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${
                filterStatus === s ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5 border-b border-white/5">
                <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest">Giao dịch</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">Khách hàng</th>
                ...
                <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Ngày thực hiện</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.03]">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-20 text-center text-slate-500 italic">Không có giao dịch nào được tìm thấy</td>
                </tr>
              ) : (
                paginated.map((t) => (
                  <tr 
                  ...
                  >
                    <td className="px-6 py-5">
                      <div className="font-mono font-bold text-indigo-400 text-sm group-hover:text-indigo-300 transition-colors">#{t.id.slice(0, 8)}</div>
                      <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-0.5">Đặt vé: {t.bookingId.slice(0, 8)}</div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="font-bold text-slate-200 text-sm group-hover:text-white transition-colors">{t.userName}</div>
                      <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{t.userEmail}</div>
                    </td>
                    <td className="px-6 py-5 text-right">
                      <div className="font-black text-white text-base tracking-tighter tabular-nums">{formatCurrency(t.amount)}</div>
                      <div className="text-[10px] font-black text-indigo-300/50 uppercase tracking-widest">{t.method}</div>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${getStatusBadge(t.status)}`}>
          ...
          >
            ...
            <div className="px-8 py-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-black text-white tracking-tight">Chi tiết giao dịch</h3>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">ID: <span className="text-indigo-400 font-mono tracking-normal">#{selectedTransaction.id}</span></p>
                </div>
              </div>
              <button 
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="text-center py-6 bg-white/[0.02] border border-white/5 rounded-3xl relative overflow-hidden group">
                ...
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 relative z-10">Số tiền giao dịch</p>
                <h4 className="text-4xl font-black text-white tracking-tighter tabular-nums relative z-10">
                  {formatCurrency(selectedTransaction.amount)}
                </h4>
                <div className="mt-4 flex justify-center relative z-10">
                  <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${getStatusBadge(selectedTransaction.status)}`}>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Phương thức</p>
                  <p className="text-sm font-bold text-slate-200">{selectedTransaction.method}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Thời gian</p>
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-500">{selectedTransaction.userEmail}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Mã đặt chuyến</p>
                  <p className="text-sm font-bold text-indigo-400 font-mono">#{selectedTransaction.bookingId}</p>
                </div>
              </div>
              ...
              <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-2xl p-4">
                <div className="flex gap-4 items-start">
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
                    Giao dịch này được tự động ghi nhận vào hệ thống sau khi khách hàng hoàn tất cổng thanh toán. Liên hệ bộ phận kỹ thuật nếu có sai lệch về số dư.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="px-8 pb-8 flex gap-3">
              <button 
                className="flex-1 px-6 py-3 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white border border-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95"
              >
                In hoá đơn
              </button>
              {selectedTransaction.status === 'completed' && (
                <button 
                  className="flex-1 px-6 py-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95"
                >
                  Hoàn tiền
                </button>
              )}
