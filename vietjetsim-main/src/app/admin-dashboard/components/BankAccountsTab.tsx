...
export default function BankAccountsTab({ onToast }: { onToast?: ToastAPI }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        onToast?.success('Thành công', editingAccount ? 'Cập nhật tài khoản thành công' : 'Thêm tài khoản mới thành công');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Thiết lập các tài khoản nhận thanh toán từ khách hàng</p>
        </div>
        <button
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 h-48 animate-pulse" />
          ))}
        </div>
      ) : accounts.length === 0 ? (
        <div className="bg-slate-800/30 border border-slate-700 border-dashed rounded-3xl p-12 text-center">
          ...
          <p className="text-slate-400 mt-2">Hãy thêm tài khoản ngân hàng đầu tiên để nhận tiền từ khách.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {accounts.map(account => (
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Số tài khoản</label>
                  <p className="text-xl font-mono font-bold text-white tracking-wider">{account.account_number}</p>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Chủ tài khoản</label>
                  <p className="text-sm font-semibold text-slate-200">{account.account_holder.toUpperCase()}</p>
                </div>
                <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/50">
                  <label className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest block mb-1">Nội dung chuyển khoản mẫu</label>
                  <p className="text-[11px] font-mono text-indigo-200/70">{account.transfer_note_template || 'VJ {code}'}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 sm:col-span-1">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Tên ngân hàng</label>
                  <input 
                    type="text" 
                    required
                ...
                <div className="col-span-2 sm:col-span-1">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Mã BIN (Tùy chọn)</label>
                  <input 
                    type="text" 
              ...
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Số tài khoản</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Chủ tài khoản</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Chi nhánh (Tùy chọn)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Nội dung Chuyển khoản mẫu</label>
                  <input 
                    type="text" 
                    value={formData.transfer_note_template}
                    onChange={e => setFormData({...formData, transfer_note_template: e.target.value})}
                    placeholder="VD: VJ {code}"
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 transition-all font-mono"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">Sử dụng `{`{code}`}` để tự động thay thế bằng mã đặt vé.</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 py-2">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <span className="text-sm text-slate-300 group-hover:text-white transition-colors">Đặt làm tài khoản mặc định</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input 
                    type="checkbox" 
                    ...
                    className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-sm text-slate-300 group-hover:text-white transition-colors">Kích hoạt tài khoản</span>
                </label>
              </div>
