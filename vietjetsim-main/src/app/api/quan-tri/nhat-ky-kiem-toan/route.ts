...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action && userId && resource && dateFrom && dateTo) {
      logs = await sql`
        SELECT 
          ...
          u.role as user_role
        FROM audit_logs al
        LEFT JOIN user_profiles u ON al.admin_id = u.id
        WHERE al.action ILIKE ${'%' + action + '%'}
          AND al.admin_id = ${userId}
          AND al.target_type ILIKE ${'%' + resource + '%'}
    ...
    const resources = await sql`SELECT DISTINCT target_type FROM audit_logs ORDER BY target_type`;
