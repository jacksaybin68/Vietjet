...
const STATUS_LABELS: Record<RecentBooking['status'], { label: string; color: string; bg: string }> = {
  pending: { label: 'Chờ thanh toán', color: 'text-[var(--vj-red)]', bg: 'bg-[var(--vj-yellow)]/25' },
  confirmed: { label: 'Đã xác nhận', color: 'text-[var(--vj-red)]', bg: 'bg-[var(--vj-red)]/10' },
  completed: { label: 'Hoàn thành', color: 'text-[var(--vj-blue)]', bg: 'bg-[var(--vj-blue)]/10' },
  cancelled: { label: 'Đã hủy', color: 'text-[var(--foreground-muted)]', bg: 'bg-[var(--foreground-muted)]/15' },
  refunded: { label: 'Đã hoàn tiền', color: 'text-[var(--vj-purple)]', bg: 'bg-[var(--vj-purple)]/15' },
};
...
export default function MyFlightsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const pendingFlights = bookings.filter(b => b.status === 'pending' || b.status === 'confirmed').length;
  ...
  const handleSearchBooking = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/checkin?bookingCode=${encodeURIComponent(bookingCode)}&lastName=${encodeURIComponent(surname)}&firstName=${encodeURIComponent(givenName)}`);
      const data = await res.json();
      if (data.success) {
        toast.success('Đặt chỗ tìm thấy!', `Mã: ${bookingCode}`);
        router.push(`/lam-thu-tuc?code=${bookingCode}`);
      } else {
        toast.show({ title: 'Không tìm thấy đặt chỗ', message: data.message || 'Vui lòng kiểm tra mã đặt chỗ và họ tên bạn' });
      }
    } catch {
    ...
    <div className="min-h-screen bg-[var(--background)] dark:bg-[var(--dark-bg)]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          {TABS.map(t => (
            <button
              ...
              className={`px-4 py-2.5 text-sm font-semibold transition-colors ${
                tab === t.id ? 'text-[var(--vj-red)] dark:text-[var(--vj-red)] border-b-2 border-[var(--vj-red)]' : 'text-[var(--foreground-muted)] hover:text-[var(--foreground)] border-b-2 border-transparent'
              }`}
            >
              {t.label}
              {bookings.length > 0 && tab === t.id && (
                <span className="ml-1.5 text-xs px-1.5 py-0.5 rounded-full bg-[var(--vj-red)]/10 text-[var(--vj-red)]">{bookings.length}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {tab === 'booking' && (
          <section>
            {bookingsLoading ? (
              <div className="space-y-4">{[1, 2, 3].map(i => <div key={i} className="h-24 rounded-xl bg-[var(--surface)] animate-pulse" />)}</div>
            ) : bookings.length > 0 ? (
              <div className="space-y-5">
                {bookings.map((b) => {
                  ...
                  return (
                    <article key={b.id} className="overflow-hidden rounded-2xl border border-[var(--border)] dark:border-[var(--dark-border)] bg-white dark:bg-[var(--dark-surface)] shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-[var(--vj-navy)] text-white">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm font-black text-[var(--vj-yellow)]">{f.flight_no || '—'}</p>
                      ...
                      <div className="px-4 py-5">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                          ...
                          <div className="text-center sm:text-left w-24">
                            <p className="text-2xl font-black text-[var(--foreground)]">{f.from_code || '--'}</p>
                            <p className="text-[10px] text-[var(--foreground-muted)] mt-0.5">{AIRPORTS[f.from_code || '']?.split('(')[0] || '—'}</p>
                            <p className="text-sm font-bold text-[var(--vj-red)] mt-1">{formatTime(f.depart_time)}</p>
                          </div>

                          {/* Flight line */}
                          <div className="flex-1 text-center">
                            <p className="text-[10px] text-[var(--foreground-muted)]">{formatDate(f.depart_time)}</p>
                          ...
                          <div className="text-center sm:text-right w-24">
                            <p className="text-2xl font-black text-[var(--foreground)]">{f.to_code || '--'}</p>
                            <p className="text-[10px] text-[var(--foreground-muted)] mt-0.5">{AIRPORTS[f.to_code || '']?.split('(')[0] || '—'}</p>
                            <p className="text-sm font-bold text-[var(--vj-red)] mt-1">{formatTime(f.arrive_time)}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {b.seat_number ? (
                            <span className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-lg bg-[var(--vj-blue)]/10 text-[var(--vj-blue)]">
                              <MdAirlineSeatReclineNormal className="h-3.5 w-3.5" /> Ghế {b.seat_number}
                            </span>
                          ) : null}
                          {b.has_check_in ? (
                            <span className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-lg bg-[var(--vj-green)]/10 text-[var(--vj-green)]">
                              <MdCheckCircle className="h-3.5 w-3.5" /> Đã check-in{b.check_in_number ? ` · ${b.check_in_number}` : ''}
                            </span>
                          ) : null}
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          <p className="text-xs text-[var(--foreground-muted)]">Đặt: {new Date(b.created_at).toLocaleDateString('vi-VN')}</p>
                          <p className="text-xl font-black text-[var(--vj-red)]">{b.total_price.toLocaleString('vi-VN')}₫</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-2 px-4 py-2 bg-[var(--surface)] dark:bg-[var(--dark-surface)] border-t">
                        <div className="flex gap-2">
                          {!b.has_check_in && b.status === 'confirmed' && (
                            <Link href={`/lam-thu-tuc?code=${b.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-xl bg-[var(--vj-yellow)] text-[var(--vj-red)] font-black hover:bg-[var(--vj-yellow-2)] transition-colors">
                              <MdAirlineSeatReclineNormal className="h-3.5 w-3.5" /> Check-in
                            </Link>
                          )}
                          {b.has_check_in && (
                            <Link href={`/lam-thu-tuc?code=${b.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-[var(--vj-yellow)]/25 text-[var(--vj-red)] font-bold hover:bg-[var(--vj-yellow)]/40 transition-colors">
                              ✅ Thẻ lên tàu
                            </Link>
                          )}
                          <Link href={`/lam-thu-tuc?code=${b.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-[var(--border)] font-semibold hover:bg-[var(--vj-yellow)]/10 transition-colors">
                            <MdPrint className="h-3.5 w-3.5" /> In vé / Đổi
                          </Link>
                        </div>
                        <Link href={`/lam-thu-tuc?code=${b.id}`} className="text-xs text-[var(--vj-red)] font-bold hover:underline">Xem chi tiết →</Link>
                      </div>
                    </article>
                  );
                })}
              ...
              <section className="vj-card mx-auto max-w-[900px] p-10 text-center">
                ...
                <p className="text-sm text-[var(--foreground-muted)] mt-2 mb-6">
                  Các chuyến bay bạn đặt sẽ hiển thị ở đây. Đặt vé đầu tiên để bắt đầu hành trình của bạn.
                </p>
                <Link href="/tim-ve" className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[var(--vj-yellow)] text-[var(--vj-red)] font-black hover:bg-[var(--vj-yellow-2)] transition-colors">
            ...
            {bookings.length > 0 ? (
              <div className="space-y-5">
                {bookings.map((b) => {
                  ...
                  return (
                    <div key={b.id} className="rounded-2xl border border-[var(--border)] dark:border-[var(--dark-border)] bg-white dark:bg-[var(--dark-surface)] p-5">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <p className="text-lg font-black text-[var(--vj-yellow)]">{vdt}</p>
                          <p className="text-[11px] text-[var(--foreground-muted)]">{f.flight_no || '—'} · {f.from_code || '--'} → {f.to_code || '--'}</p>
                        </div>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-x-6 text-[13px]">
                        <div>
                          ...
                          <p className="font-medium text-[var(--foreground)]">{formatDate(f.depart_time)}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[var(--foreground-muted)]">Giờ</p>
                          <p className="font-medium text-[var(--foreground)]">{formatTime(f.depart_time)} - {formatTime(f.arrive_time)}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[var(--foreground-muted)]">Trạng thái</p>
                          <p className="font-medium text-[var(--foreground)]">{b.status === 'completed' ? 'Hoàn thành' : b.status === 'confirmed' ? 'Đã xác nhận' : st.label}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-[var(--foreground-muted)]">Giá</p>
                          <p className="font-black text-[var(--vj-red)]">{b.total_price.toLocaleString('vi-VN')}₫</p>
                        </div>
                      </div>
                      <div className="mt-4 flex gap-2">
                        <button onClick={() => window.print()} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-xl bg-[var(--vj-yellow)] text-[var(--vj-red)] font-black hover:bg-[var(--vj-yellow-2)] transition-colors">
                          <MdPrint className="h-3.5 w-3.5" /> In vé
                        </button>
                        <Link href={`/lam-thu-tuc?ticket=${b.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-[var(--border)] font-semibold hover:bg-[var(--vj-yellow)]/10 transition-colors">
              ...
              <section className="vj-card p-10 text-center">
                ...
                <p className="text-sm text-[var(--foreground-muted)]">Vé điện tử của đặt chỗ sẽ hiển thị tại đây.</p>
              </section>
            )}
          </section>
        )}
