...
export default function HeroSection() {
  ...
  // Date inputs start empty so server and first client render match; the real
  // defaults (today / +7 days) are applied after mount — deriving them during
  // render would reintroduce the hydration mismatch fixed in DealCard.
  useEffect(() => {
    const iso = (d: Date) =>
    ...
    const plus7 = iso(plus7Date);

    const depart = departRef.current;
    if (depart) {
      depart.min = today;
      if (!depart.value || depart.value < today) depart.value = today;
    }
    const ret = returnRef.current;
    if (ret) {
      ret.min = depart?.value || today;
      if (!ret.value || ret.value < ret.min) ret.value = ret.min <= plus7 ? plus7 : ret.min;
    }
  }, []);

  const search = (event: FormEvent) => {
    event.preventDefault();
    const query = new URLSearchParams({ from, to, pax: String(pax.adults) });
    ...
    router.push(`/tim-ve?${query.toString()}`);
  };

  return (
    <section
      className="relative min-h-[565px] overflow-hidden bg-cover bg-center py-10 lg:min-h-[610px] lg:py-14  :py-16"
      style={{
        backgroundImage:
    ...
    >
      <div className="mx-auto max-w-[1240px] px-4">
        <div className="max-w-xl pt-5 font-[var(--vj-font)] text-white lg:pt-10 animate-fade-in-up">
          ...
          <form
            onSubmit={search}
            className="mt-6 w-full max-w-[540px] bg-[#EC2029] [#D91A21] border-2 border-[#EC2029] [#D91A21] rounded-lg shadow-sm p-4 sm:p-6 font-[var(--vj-font)]"
          >
            ...
            <div className="mt-4 space-y-2 md:space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-2 md:gap-3">
                <div className="flex h-[50px] md:h-[58px] items-center rounded-xl bg-white  border-2 border-[#EC2029] [#444] px-3 md:px-4 text-left shadow-sm">
                  <MdFlightTakeoff className="text-[24px] md:text-[32px] text-black /80" />
                  <div className="ml-2 md:ml-3 min-w-0 flex-1">
                    <div className="text-[8px] md:text-[10px] font-bold uppercase tracking-[0.08em] text-[#1A2948] /70">
                      Điểm khởi hành
                    </div>
                    <select
                      value={from}
                      onChange={(e) => setFrom(e.target.value)}
                      className="mt-1 w-full appearance-none bg-transparent text-[14px] md:text-[19px] font-black text-black  outline-none sm:text-[16px] md:text-[20px]"
                    >
                      {airports.map((item) => (
                        <option
                          key={item.code}
                          value={item.code}
                          className="bg-white  text-[#1A2948] "
                        >
                          {item.city} ({item.code})
                        </option>
                      ))}
                ...
                <div className="flex h-[50px] md:h-[58px] items-center rounded-xl bg-white  border-2 border-[#EC2029] [#444] px-3 md:px-4 text-left shadow-sm">
                  <div className="ml-2 md:ml-3 min-w-0 flex-1">
                    ...
                    <input
                      type="date"
                      ref={departRef}
                      className="mt-1 w-full bg-transparent text-[12px] md:text-[15px] font-black text-black  outline-none sm:text-[14px] md:text-[16px]"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-[1fr_180px] gap-2 md:gap-3">
                <div className="flex h-[50px] md:h-[58px] items-center rounded-xl bg-white  border-2 border-[#EC2029] [#444] px-3 md:px-4 text-left shadow-sm">
                  <MdFlightLand className="text-[24px] md:text-[32px] text-black /80" />
                  <div className="ml-2 md:ml-3 min-w-0 flex-1">
                    <div className="text-[8px] md:text-[10px] font-bold uppercase tracking-[0.08em] text-[#1A2948] /70">
                      Điểm đến
                    </div>
                ...
                <div className="flex h-[50px] md:h-[58px] items-center rounded-xl bg-white  border-2 border-[#EC2029] [#444] px-3 md:px-4 text-left shadow-sm">
                  <div className="ml-2 md:ml-3 min-w-0 flex-1">
                    ...
                    <input
                      ...
                      ref={returnRef}
                      disabled={!roundTrip}
                      className="mt-1 w-full bg-transparent text-[12px] md:text-[15px] font-black text-black  outline-none disabled:text-[#999] :text-[#666] sm:text-[14px] md:text-[16px]"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="mt-2 md:mt-4">
                <div
                  ...
                  className={`cursor-pointer flex items-center justify-between px-3 md:px-4 py-2 md:py-3 bg-white  border-2 border-[#EC2029] [#444] hover:bg-[#f9f9f9] :bg-[#2a2a2a] transition-colors duration-200 h-[46px] md:h-[58px] ${
                    paxOpen ? 'rounded-t-xl border-b-0' : 'rounded-xl'
                  }`}
                >
                  <div className="flex items-center gap-2 md:gap-3">
                    <MdPeople className="h-4 w-4 md:h-5 md:w-5 text-black [#FFDD00]" />
                    <div>
                      <div className="text-[10px] font-bold uppercase tracking-wide text-black opacity-70 /70">
                        Hành khách
                      </div>
                ...
                >
                  <div className="px-3 md:px-4 py-2 bg-white  border-2 border-t-0 border-[#EC2029] [#444]  rounded-b-xl">
                    <div className="grid gap-2 md:gap-4">
                      {/* Adults Row */}
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 md:gap-3">
                          <MdPerson className="h-4 w-4 md:h-5 md:w-5 scale-[0.85] text-black /80" />
                          <div>
                            <div className="text-[11px] md:text-[13px] font-bold text-[#1A2948] ">
                              Người lớn
                            </div>
                            <div className="text-[9px] md:text-[11px] text-[#1A2948] /70">
                              {'> 12 tuổi'}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 md:gap-2">
                          <button
                            ...
                            className="flex h-6 w-6 md:h-8 md:w-8 items-center justify-center rounded-full border border-[#d1d5db] [#555] text-[10px] md:text-[12px] text-[#6B7280] /60 hover:bg-[#f3f4f6] :bg-[#333]"
                          >
                            -
                          </button>
                          <span className="text-[14px] md:text-[16px] font-bold text-[#1A2948] ">
                            {pax.adults}
                          </span>
                          <button
                            onClick={() =>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 md:gap-3">
                          <MdChildCare
                            viewBox="0 0 24 24"
                            className="h-4 w-4 md:h-5 md:w-5 scale-[0.85] text-black /80"
                          />
                          <div>
                            <div className="text-[11px] md:text-[13px] font-bold text-[#1A2948] ">
                              Trẻ em
              ...
              {/* Discount Code Input */}
              <div className="mt-2 md:mt-4 flex items-center gap-2 rounded-xl bg-white  border-2 border-[#EC2029] [#444] px-3 md:px-4 h-[46px] md:h-[58px]">
                <MdConfirmationNumber className="text-[16px] md:text-[20px] text-black /80" />
                <input
                  type="text"
                  placeholder="Nhập mã giảm giá"
                  className="mt-0 w-full bg-transparent text-[13px] md:text-[17px] font-black text-black  outline-none sm:text-[15px] md:text-[18px]"
                />
              </div>
              ...
              <label className="mt-2 flex items-center gap-2 text-[12px] md:text-[14px] font-bold text-white">
                <input
                  type="checkbox"
                  className="h-4 w-4 md:h-5 md:w-5 rounded border-0 accent-[#f3c84d] [#FFDD00]"
                />
