...
function MetadataBadge({ type, metadata }: { type: string; metadata?: Record<string, string> }) {
  ...
  if (type === 'flight' && metadata.delay_type) {
    ...
    return (
      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-50 /10 text-red-600  border border-red-100 ">
  ...
  if (type === 'promo' && metadata.promo_code) {
    return (
      <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-purple-50 /10 text-purple-700  border border-purple-100  tracking-wide">
        {metadata.promo_code}
      </span>
    );
  }
  if (type === 'promo' && metadata.expires_hours) {
    return (
      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-50 /10 text-amber-700  border border-[rgb(var(--accent-rgb))]/20 ">
  ...
  if (type === 'booking' && metadata.booking_id) {
    return (
      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-50 /10 text-green-700  border border-green-100 ">
        #{metadata.booking_id}
      </span>
    );
  }
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span
                className={`text-sm font-bold truncate ${notif.read ? 'text-[var(--foreground-muted)] [var(--foreground-muted)]' : 'text-[var(--vj-navy)] '}`}
              >
                {notif.title}
              </span>
            </div>
            <span
              className="flex-shrink-0 text-xs text-[var(--foreground-subtle)] [var(--foreground-subtle)] whitespace-nowrap"
          ...
          {/* Body */}
          <p className="text-xs text-[var(--foreground-muted)] [var(--foreground-muted)] mt-1 leading-relaxed line-clamp-2">
            {notif.body}
          </p>
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {isSnoozed && (
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-50 /10 text-amber-600  flex items-center gap-1">
    ...
    >
      ...
      <div className="px-5 py-4 border-b border-[var(--border)]">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-bold text-base leading-tight text-[var(--foreground)] [var(--foreground)]">
                Trung tâm thông báo
              </h2>
              <p className="text-xs text-[var(--foreground-subtle)] [var(--foreground-subtle)]">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button
                ...
                className="text-xs font-semibold text-[var(--foreground-muted)] [var(--foreground-muted)] hover:text-primary transition-colors flex items-center gap-1"
              >
            ...
            <button
              ...
              className="text-[var(--foreground-subtle)] [var(--foreground-subtle)] hover:text-primary transition-colors"
              title="Tải lại"
            >
        ...
        <div className="relative mb-3">
          <Icon
            ...
            size={14}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--foreground-subtle)] [var(--foreground-subtle)]"
          />
          <input
            type="text"
            value={searchQuery}
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all flex-shrink-0 ${
                  activeType === type
                    ...
                    : 'text-[var(--foreground-muted)] [var(--foreground-muted)] bg-[var(--surface-2)] hover:bg-[var(--surface)] '
                }`}
        ...
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          <select
            ...
            className="text-xs border border-[var(--border)] rounded-lg px-2 py-1.5 bg-[var(--surface)] text-[var(--foreground-muted)] [var(--foreground-muted)] focus:outline-none"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-xl border transition-all ${
              showUnreadOnly
                ...
                : 'text-[var(--foreground-muted)] [var(--foreground-muted)] border-[var(--border)] hover:bg-[var(--surface-2)] '
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-xl border transition-all ${
              showArchived
                ? 'bg-[var(--surface-2)] text-[var(--foreground-muted)] [var(--foreground-muted)] border-[var(--border)] '
                : 'text-[var(--foreground-muted)] [var(--foreground-muted)] border-[var(--border)] hover:bg-[var(--surface-2)] '
            }`}
          >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-16 text-center px-5">
          ...
          <p className="font-bold text-[var(--foreground-muted)] [var(--foreground-muted)] text-sm">
            Không có thông báo
          </p>
          <p className="text-xs text-[var(--foreground-subtle)] [var(--foreground-subtle)] mt-1 max-w-xs">
